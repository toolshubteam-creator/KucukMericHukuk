...
- Default culture: `tr-TR`, supported: `tr-TR`, `en-US`
- URL pattern: culture **ZORUNLU** (geçersiz culture'da `CultureRouteConstraint` 404 verir). Public canonical route segmentleri culture-aware çalışır: Türkçe `/iletisim`, `/randevu`, `/makaleler`, `/hizmetler`, `/avukatlar`, `/sayfalar/{slug}`, `/sss`, `/galeri`, `/referanslar`, `/abone/...`; İngilizce `/contact`, `/appointment`, `/articles`, `/services`, `/attorneys`, `/pages/{slug}`, `/faq`, `/gallery`, `/testimonials`, `/subscriber/...`. Eski Türkçe yayın öncesi İngilizce controller route'ları sadece `tr-TR` için 301 ile Türkçe canonical adreslere taşınır; `en-US` canonical route'ları redirect edilmez.
...
**Mevcut Faz:** Faz 8 — Yayın Öncesi Stabilizasyon & Cila (🔄 devam ediyor). Faz 6 deploy bekliyor (kod-tamamlandı); yayın tarihi yaklaşınca Faz 6 + Faz 7 + Faz 8 birlikte 6.26 deploy turu ve `v0.7.0` tag hazırlığına alınacak.
...
- 🔄 **Faz 6 — Test, Düzeltme, Yayına Hazırlık** devam ediyor (6.1–6.25 tamam, 6.26 deploy bekliyor)
- ✅ **Faz 7 — Admin Zenginleştirme** tamamlandı (20.05.2026): 7.1 Aktivite Logu + 7.1.2 Translation merge + 7.2 Aboneler+Bülten + 7.3 404 Takibi + 7.4 Redirect + 7.5 Google ortak altyapı + 7.6 GA4 dashboard widget + 7.7 Search Console dashboard widget + 7.8 kapanış; **637/637 test PASSED** (501 unit + 136 integration)
- 🔄 **Faz 8 — Yayın Öncesi Stabilizasyon & Cila** devam ediyor (20.05.2026 başladı): admin topbar dropdown + Turnstile JS centralize + SEO meta UI keşfi + public route Türkçeleştirme + İngilizce route/translation altyapısı tamam; **671/671 test PASSED** (501 unit + 170 integration)
